...
**Retirement plan:** three hooks — `useCrudApi(endpoint)`, `useModalForm(initial)`, `useTableFilter(items, keys)` — extracted from the _members_ page first (largest, most complete), then rolled to the other three. Expected: each page drops to ~300–350 lines, new admin pages become composition instead of copying.
...
- PATCH whitelist in `api/membership/[id]/route.ts:19`: `['student','young-doctor']` — matches _neither_.
...
- **Validation is hand-rolled per route** (or absent); `zod@4` is installed and _unused_. → shared schemas in `src/lib/schemas.ts`, `safeParse` at every POST/PATCH boundary. This also closes several security findings (S2 mass-assignment, archive PATCH limits).
...
- Success page promises an email ("Vous recevrez votre carte membre par email") that the system cannot send — no email subsystem exists. This is a _product_ debt until the card generator ships.
