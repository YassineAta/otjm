...
import { EASE, fadeUp, fadeUpReduced, stagger } from '@/lib/animations'
...
const BADGE_BG: Record<Category, string> = {
  protests: 'border border-red-400/30',
  statements: 'border border-blue-400/30',
  announcements: 'border border-amber-400/30',
  updates: 'border border-slate-400/30',
}
...
function CategoryBadge({ category }: { category: Category }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-xs font-semibold uppercase tracking-wider px-2 py-0.5 rounded ${BADGE_BG[category]} ${c.color}`}
    >
...
function FeaturedCard({ item, onClick }: { item: NewsItem; onClick: () => void }) {
  ...
  return (
    <motion.article
      variants={fadeUp}
      onClick={onClick}
      className="group cursor-pointer col-span-2 relative overflow-hidden rounded-xl bg-[var(--otjm-ink)] text-white"
      style={{ minHeight: 420 }}
    >
      <div className="absolute inset-0">
        <img
          src={item.imageUrl || '/otjmlogo.jpg'}
          alt={item.title}
          loading="lazy"
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
        ...
        <h2 className="font-editorial text-2xl md:text-3xl font-bold mt-3 mb-3 leading-tight group-hover:text-[var(--otjm-red)] transition-colors">
          {item.title}
        </h2>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="flex items-center gap-1 text-sm font-semibold text-[var(--otjm-red)] group-hover:gap-2 transition-all">
            {t.home.readArticle} <ArrowRight className="w-4 h-4 rtl-flip" />
          </span>
        </div>
      </div>
    </motion.article>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-1 p-5">
        <CategoryBadge category={item.category} />
        <h3
          className="font-editorial text-lg font-bold mt-2 mb-2 leading-snug group-hover:text-[var(--otjm-red)] transition-colors line-clamp-3"
...
function CompactCard({ item, onClick }: { item: NewsItem; onClick: () => void }) {
  ...
  return (
    <motion.article
      onClick={onClick}
      className="group cursor-pointer flex items-center gap-4 py-3 px-4"
      whileHover={{ x: 4 }}
      transition={{ type: 'spring', stiffness: 400, damping: 30 }}
      style={{ borderInlineStart: `3px solid ${c.hex}` }}
    >
      <div className="flex-1 min-w-0">
        <span className={`text-[10px] font-bold uppercase tracking-wider ${c.color}`}>
          {t.categories[item.category]}
        </span>
        <h4 className="font-semibold text-sm mt-0.5 leading-snug line-clamp-2 group-hover:text-[var(--otjm-red)] transition-colors text-[var(--otjm-text)]">
          {item.title}
        </h4>
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        <SectionReveal>
          <motion.div variants={fadeUp} className="mb-14">
            <span className="text-[var(--otjm-red)] text-sm font-bold uppercase tracking-widest">
              {t.home.membershipLabel}
            </span>
            <h2
              className="font-editorial text-4xl md:text-5xl font-bold mt-2 mb-4 leading-tight"
              style={{ color: 'var(--otjm-text)' }}
            >
          ...
          <motion.div variants={fadeUp} className="grid sm:grid-cols-2 gap-5 max-w-2xl">
            {tiers.map((tier, tierIdx) => {
              ...
              return (
                <motion.div
                  key={tier.role}
                  className="relative rounded-2xl p-6 border transition-colors duration-300 group"
                  style={{
                ...
                >
                  ...
                  <div className="flex items-baseline gap-1 mb-1">
                    ...
                    <span className="text-sm" style={{ color: 'var(--otjm-text-muted)' }}>
                      {t.home.perYear}
                    </span>
                  </div>
                  <div
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="mt-4 space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-medium text-foreground mb-1 block">
                      {t.home.firstNameLabel}
                    </label>
                    <Input required />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-foreground mb-1 block">
                      {t.home.lastNameLabel}
                    </label>
                    <Input required />
                  </div>
                </div>
                <div>
                  ...
                  <Input type="email" name="email" placeholder="amine@chu-tunis.tn" required />
                </div>
                <div>
                  <label className="text-xs font-medium text-foreground mb-1 block">
                    {t.home.statusLabel}
                ...
                <div className="flex items-start gap-2.5 pt-1">
                  ...
                  >
                    ...
                    <Link
                      href="/privacy"
                      target="_blank"
                      className="underline underline-offset-2 hover:text-[var(--otjm-red)] transition-colors"
                      onClick={(e) => e.stopPropagation()}
                    >
...
export default function OTJMHome() {
  ...
  useEffect(() => {
    ...
    ;['accueil', 'actualites', 'membership', 'contact'].forEach((id) => {
      ...
      const obs = new IntersectionObserver(
        ([entry]) => {
          if (entry.isIntersecting) setActiveSection((prev) => (prev === id ? prev : id))
        },
        { threshold: 0.3 },
      )
      obs.observe(el)
  ...
  const filteredNews =
    newsFilter === 'all' ? newsItems : newsItems.filter((n) => n.category === newsFilter)
  ...
  return (
    ...
    >
      <div
        className="bg-[var(--otjm-red)] text-white text-xs font-semibold py-1.5 overflow-hidden cursor-pointer"
        onMouseEnter={() => setTickerPaused(true)}
        onMouseLeave={() => setTickerPaused(false)}
        style={{ userSelect: 'none' }}
      >
        <div className="flex items-center relative">
          <span className="shrink-0 bg-white text-[var(--otjm-red)] px-3 py-0.5 uppercase tracking-widest text-xs font-black relative z-10">
            Flash
          </span>
          <div
            className="absolute left-[4.5rem] top-0 bottom-0 w-6 z-[5] pointer-events-none"
            style={{ background: 'linear-gradient(to right, var(--otjm-red), transparent)' }}
          />
          <motion.div
            className="flex gap-16 whitespace-nowrap ml-4"
          ...
          <div className="flex items-center justify-between h-16">
              ...
              {navItems.map((item) => {
                const cls = `px-4 py-2 rounded-md text-sm font-medium transition-colors ${
                  activeSection === item.id
                    ? 'text-[var(--otjm-red)] font-semibold'
                ...
                return item.href ? (
                  ...
                  <button key={item.id} onClick={() => handleNavClick(item.id)} className={cls}>
                    {item.label}
                  </button>
                )
              })}
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                asChild
                size="sm"
                className="hidden md:inline-flex bg-white text-[var(--otjm-red)] font-bold hover:bg-white/90"
              >
              ...
              <div className="container mx-auto px-4 py-3 space-y-1">
                ...
                <div className="pt-2 pb-1 flex flex-col gap-2">
                  <Button
                    ...
                    size="sm"
                    variant="outline"
                    className="w-full text-white border-white/20 font-bold"
                  >
                    {t.header.langToggle}
                  </Button>
                  <Button
                    ...
                    size="sm"
                    variant="outline"
                    className="w-full text-white border-white/20 gap-2"
                  >
                    {isDark ? (
                      <>
                        <Sun className="w-4 h-4" /> {t.home.lightMode}
                      </>
                    ) : (
                      <>
                        <Moon className="w-4 h-4" /> {t.home.darkMode}
                      </>
                    )}
                  </Button>
                  <Button
                  ...
                  <Button
                    asChild
                    className="w-full bg-white text-[var(--otjm-red)] font-bold hover:bg-white/90"
                    size="sm"
                  >
          ...
          >
            ...
            >
              {headlineParts.map((part, pi) =>
                part.text.split(/(\s+)/).map((token, ti) =>
                  /^\s+$/.test(token) ? (
                    <span key={`${pi}-${ti}`} style={{ display: 'inline', whiteSpace: 'pre' }}>
                      {token}
                    </span>
                  ) : (
                    <span
            ...
            >
              ...
              <Button
                ...
                size="lg"
                variant="outline"
                className="font-semibold text-white border-white/30 hover:bg-white/10 bg-transparent"
              >
                {t.home.joinOtjm}
              </Button>
            </motion.div>
          ...
          <SectionReveal>
            <motion.div variants={fadeUp} className="flex items-end justify-between mb-10">
              <div>
                ...
                <h2
                  className="font-editorial text-3xl md:text-4xl font-bold mt-1"
                  style={{ color: 'var(--otjm-text)' }}
                >
              ...
              <Link
                href="/archives"
                className="hidden md:flex items-center gap-1 text-sm font-medium text-[var(--otjm-muted)] hover:text-[var(--otjm-red)] transition-colors"
              >
            ...
            <motion.div variants={fadeUp} className="relative mb-8">
              <div className="flex gap-2 overflow-x-auto scrollbar-none pb-1">
                {(['all', 'protests', 'statements', 'announcements', 'updates'] as const).map(
                  (f) => {
                ...
                {gridItems.length > 0 && (
                  <div>
                    <div className="flex items-center gap-4 mb-4">
                      <div
                        className="h-px flex-1"
                        style={{ background: isDark ? 'rgba(255,255,255,0.1)' : '#E5E1DC' }}
                      />
                      <span
                        className="text-xs font-bold uppercase tracking-widest"
                        style={{ color: 'var(--otjm-text-muted)' }}
              ...
              <div className="flex flex-col sm:flex-row gap-3 shrink-0">
                ...
                >
                  <Button
                    ...
                    size="lg"
                    variant="outline"
                    className="border-white/40 text-white hover:bg-white/10 font-semibold bg-transparent"
                  >
                    {t.header.newsletter}
                  </Button>
                </motion.div>
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2 text-sm text-white/60">
                  ...
                  <a href="tel:+21671414095" className="hover:text-white transition-colors">
                    +216 71 414 095
                  </a>
                  <span className="text-white/30">/</span>
                  <a href="tel:+21698655883" className="hover:text-white transition-colors">
                    +216 98 655 883
                  </a>
                </div>
                <div className="flex items-center gap-2 text-sm text-white/60">
                  ...
                  <a
                    href="mailto:otjm.national@gmail.com"
