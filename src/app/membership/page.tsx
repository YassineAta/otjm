...
import {
  GraduationCap, Stethoscope, Check, ChevronDown,
  ChevronRight, ExternalLink, ArrowRight, Users,
} from 'lucide-react'
    ...
    <div className={`min-h-screen font-body ${isDark ? '' : 'light-mode'}`} style={{ background: 'var(--otjm-bg)' }}>
      ...
      <section className="relative bg-[var(--otjm-dark)] text-white overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-5">
          <div className="absolute inset-0" style={{ backgroundImage: 'radial-gradient(var(--otjm-red) 1px, transparent 1px)', backgroundSize: '32px 32px' }} />
        </div>
        <div className="relative container mx-auto px-4 max-w-6xl py-20">
          <motion.div initial="hidden" animate="visible" variants={stagger} className="max-w-2xl">
            <motion.div variants={fadeUp} className="flex items-center gap-2 mb-4">
              ...
              <span className="text-white/50 text-xs uppercase tracking-widest font-semibold">{m.eyebrow}</span>
            </motion.div>
            <motion.h1 variants={fadeUp} className="font-editorial text-5xl md:text-6xl font-black leading-tight mb-4">
              {m.title}<br /><span className="text-[var(--otjm-red)]">{m.titleAccent}</span>
            </motion.h1>
            ...
            <motion.div variants={fadeUp} className="flex flex-wrap gap-6 text-sm text-white/50">
              <span className="flex items-center gap-2"><Users className="w-4 h-4 text-[var(--otjm-red)]" />{m.stats[0]}</span>
              <span className="flex items-center gap-2"><Check className="w-4 h-4 text-[var(--otjm-red)]" />{m.stats[1]}</span>
              <span className="flex items-center gap-2"><Check className="w-4 h-4 text-[var(--otjm-red)]" />{m.stats[2]}</span>
            </motion.div>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 max-w-3xl">
          ...
          >
            <motion.div variants={fadeUp} className="mb-12">
              <span className="text-[var(--otjm-red)] text-xs font-bold uppercase tracking-widest">{m.stepsLabel}</span>
              <h2 className="font-editorial text-3xl md:text-4xl font-bold mt-2" style={{ color: 'var(--otjm-text)' }}>
                {m.stepsTitle}
              </h2>
            </motion.div>
                    ...
                    >
                      <div
                        ...
                        style={{
                          ...
                          background: isDone ? 'var(--otjm-red)' : isActive ? 'transparent' : 'var(--otjm-bg)',
                      ...
                      <div
                        ...
                        style={{
                          background: isActive
                            ? (isDark ? 'rgba(200,16,46,0.08)' : 'rgba(200,16,46,0.05)')
                      ...
                      >
                        ...
                        <AnimatePresence>
                          {isActive && (
                            ...
                            >
                              <p className="text-sm leading-relaxed mt-3 mb-4" style={{ color: 'var(--otjm-text-muted)' }}>
                                {step.desc}
                              </p>
      ...
      {/* ── Pricing tiers ── */}
      <section className="py-16" style={{ background: isDark ? 'var(--otjm-ink)' : 'var(--otjm-surface)' }}>
        <div className="container mx-auto px-4 max-w-3xl">
          <motion.div
            initial="hidden"
            whileInView="visible"
          ...
          >
            <motion.div variants={fadeUp} className="mb-10">
              <span className="text-[var(--otjm-red)] text-xs font-bold uppercase tracking-widest">{m.pricingLabel}</span>
              <h2 className="font-editorial text-3xl font-bold mt-2" style={{ color: 'var(--otjm-text)' }}>
                {m.pricingTitle}
              </h2>
            </motion.div>
              ...
              {m.tiers.map((tier, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    whileHover={{ borderColor: 'var(--otjm-red)', boxShadow: '0 0 30px rgba(200,16,46,0.15)' }}
                  ...
                  >
                    ...
                    <div className="flex items-baseline gap-1 mb-1">
                      ...
                      <span className="text-sm font-bold" style={{ color: 'var(--otjm-red)' }}>DT</span>
                      <span className="text-xs ml-1" style={{ color: 'var(--otjm-text-muted)' }}>{m.perYear}</span>
                    </div>
      ...
      <section className="py-16" style={{ background: 'var(--otjm-bg)' }}>
        <div className="container mx-auto px-4 max-w-2xl">
          ...
          >
            <motion.div variants={fadeUp} className="mb-10">
              <span className="text-[var(--otjm-red)] text-xs font-bold uppercase tracking-widest">{m.faqLabel}</span>
              <h2 className="font-editorial text-3xl font-bold mt-2" style={{ color: 'var(--otjm-text)' }}>
                {m.faqTitle}
              </h2>
            </motion.div>
            ...
            <motion.div variants={fadeUp} className="space-y-2">
              {m.faq.map((item, i) => (
                <div
                  ...
                  style={{
                    borderColor: openFaq === i ? 'var(--otjm-red)' : isDark ? 'rgba(255,255,255,0.08)' : '#E5E1DC',
                ...
                >
                  ...
                  >
                    <span className="font-semibold text-sm ltr:pr-4 rtl:pl-4" style={{ color: 'var(--otjm-text)' }}>{item.q}</span>
                    <ChevronDown
                  ...
                  <AnimatePresence>
                    {openFaq === i && (
                      <motion.div
                        ...
                        className="overflow-hidden"
                      >
                        <p className="px-5 pb-4 text-sm leading-relaxed" style={{ color: 'var(--otjm-text-muted)' }}>
                          {item.a}
                        </p>
                      </motion.div>
                    )}
